...
pub const query_t = extern struct {
    ...
    terms: [FLECS_TERM_COUNT_MAX]term_t = .{term_t{}} ** FLECS_TERM_COUNT_MAX,
...
pub fn init() *world_t {
    ...
    num_worlds += 1;
    component_ids_hm = ComponentIdHashMap.init(std.heap.c_allocator);
    ...
    std.log.debug("FLECS finished! world={}\n", .{world});
    return world;
}
...
extern fn ecs_set_id(world: *world_t, entity: entity_t, id: id_t, size: usize, ptr: ?*const anyopaque) void;
//--------------------------------------------------------------------------------------------------
//
...
const ComponentIdHashMap = std.AutoHashMap(*id_t, u0);
var component_ids_hm: ComponentIdHashMap = undefined;
