...
#define FT_DEFINE_RASTER_FUNCS(class_, glyph_format_, raster_new_,             \
  ...
  const FT_Raster_Funcs class_ = {glyph_format_,  raster_new_,                 \
                                  raster_reset_,  raster_set_mode_,            \
                                  raster_render_, raster_done_};
...
typedef void Function_Sweep_Span(RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2,
                                 PProfile left, PProfile right);
...
static Bool Finalize_Profile_Table(RAS_ARG) {
  ...
  if (n > 1 && p) {
    do {
      ...
      if (Insert_Y_Turn(RAS_VARS bottom) || Insert_Y_Turn(RAS_VARS top + 1))
        return FAILURE;
 ...
 */
static Bool Line_Up(RAS_ARGS Long x1, Long y1, Long x2, Long y2, Long miny,
                    Long maxy) {
  Long Dx, Dy;
 ...
 *   SUCCESS on success, FAILURE on render pool overflow.
 */
static Bool Line_Down(RAS_ARGS Long x1, Long y1, Long x2, Long y2, Long miny,
                      Long maxy) {
  Bool result, fresh;
 ...
 *   SUCCESS on success, FAILURE on render pool overflow.
 */
static Bool Bezier_Up(RAS_ARGS Int degree, TPoint *arc, TSplitter splitter,
                      Long miny, Long maxy) {
  Long y1, y2, e, e2, e0;
  Short f1;
  ...
  do {
    ...
    if (y2 > e) {
      ...
      if (y2 - y1 >= ras.precision_step) {
        ...
        *top++ =
            arc[degree].x + FMulDiv(arc[0].x - arc[degree].x, e - y1, y2 - y1);
        arc -= degree;
        e += ras.precision;
      }
    } else {
 ...
 *   SUCCESS on success, FAILURE on render pool overflow.
 */
static Bool Bezier_Down(RAS_ARGS Int degree, TPoint *arc, TSplitter splitter,
                        Long miny, Long maxy) {
  Bool result, fresh;
...
static Bool Line_To(RAS_ARGS Long x, Long y) {
  ...
  case Unknown_State:
    if (y > ras.lastY) {
      if (New_Profile(RAS_VARS Ascending_State, IS_BOTTOM_OVERSHOOT(ras.lastY)))
        return FAILURE;
    } else {
      if (y < ras.lastY)
        if (New_Profile(RAS_VARS Descending_State, IS_TOP_OVERSHOOT(ras.lastY)))
          return FAILURE;
    }
    break;
  ...
  case Ascending_State:
    if (y < ras.lastY) {
      if (End_Profile(RAS_VARS IS_TOP_OVERSHOOT(ras.lastY)) ||
          New_Profile(RAS_VARS Descending_State, IS_TOP_OVERSHOOT(ras.lastY)))
        return FAILURE;
    }
    break;
  ...
  case Descending_State:
    if (y > ras.lastY) {
      if (End_Profile(RAS_VARS IS_BOTTOM_OVERSHOOT(ras.lastY)) ||
          New_Profile(RAS_VARS Ascending_State, IS_BOTTOM_OVERSHOOT(ras.lastY)))
        return FAILURE;
    }
    break;
  ...
  do {
    ...
    if (y2 < ymin || y2 > ymax) {
      ...
      if (ras.state != state_bez) {
        ...
        if (ras.state != Unknown_State && End_Profile(RAS_VARS o))
          goto Fail;
      ...
      if (state_bez == Ascending_State) {
        if (Bezier_Up(RAS_VARS 2, arc, Split_Conic, ras.minY, ras.maxY))
          goto Fail;
      } else if (Bezier_Down(RAS_VARS 2, arc, Split_Conic, ras.minY, ras.maxY))
        goto Fail;
      arc -= 2;
    }
 ...
 *  profile.
 */
static Bool Cubic_To(RAS_ARGS Long cx1, Long cy1, Long cx2, Long cy2, Long x,
                     Long y) {
  ...
  do {
    ...
    if (ymin2 < ymin1 || ymax2 > ymax1) {
      ...
      if (state_bez == Ascending_State) {
        if (Bezier_Up(RAS_VARS 3, arc, Split_Cubic, ras.minY, ras.maxY))
          goto Fail;
      } else if (Bezier_Down(RAS_VARS 3, arc, Split_Cubic, ras.minY, ras.maxY))
        goto Fail;
      arc -= 3;
    }
 ...
 */
static Bool Decompose_Curve(RAS_ARGS Int first, Int last, Int flipped) {
  FT_Vector v_last;
  FT_Vector v_control;
  FT_Vector v_start;
  ...
  while (point < limit) {
    ...
    Do_Conic:
      if (point < limit) {
        ...
        if (Conic_To(RAS_VARS v_control.x, v_control.y, v_middle.x, v_middle.y))
          goto Fail;
      ...
      if (Conic_To(RAS_VARS v_control.x, v_control.y, v_start.x, v_start.y))
        goto Fail;
...
static Bool Convert_Glyph(RAS_ARGS Int flipped) {
  ...
  for (i = 0; i < ras.outline.n_contours; i++) {
    ...
    if (FRAC(ras.lastY) == 0 && ras.lastY >= ras.minY && ras.lastY <= ras.maxY)
      if (ras.gProfile &&
    ...
    if (ras.top != ras.cProfile->offset && (ras.cProfile->flags & Flow_Up))
 ...
 */
static void Init_Linked(TProfileList *l) { *l = NULL; }
 ...
 */
static void DelOld(PProfileList list, const PProfile profile) {
  PProfile *old, current;
...
static void Vertical_Sweep_Span(RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2,
                                PProfile left, PProfile right) {
  Long e1, e2;
  ...
  FT_TRACE7(("  y=%d x=[% .12f;% .12f]", y, (double)x1 / (double)ras.precision,
  ...
  if (dropOutControl != 2 && x2 - x1 - ras.precision <= ras.precision_jitter &&
      e1 != x1 && e2 != x2)
    e2 = e1;
...
static void Vertical_Sweep_Drop(RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2,
                                PProfile left, PProfile right) {
  Long e1, e2, pxl;
  Short c1, f1;
    ...
    if (e1 == e2 + ras.precision) {
      ...
      case 5: /* smart drop-outs excluding stubs  */
        ...
        if (left->next == right && left->height <= 0 &&
            !(left->flags & Overshoot_Top && x2 - x1 >= ras.precision_half))
          goto Exit;

        /* lower stub test */
        if (right->next == left && left->start == y &&
            !(left->flags & Overshoot_Bottom && x2 - x1 >= ras.precision_half))
          goto Exit;
      ...
      if (e1 >= 0 && e1 < ras.bWidth && ras.bLine[c1] & (0x80 >> f1))
        goto Exit;
    } else
      goto Exit;
  }
...
static void Vertical_Sweep_Step(RAS_ARG) { ras.bLine -= ras.target.pitch; }
...
static void Horizontal_Sweep_Span(RAS_ARGS Short y, FT_F26Dot6 x1,
                                  FT_F26Dot6 x2, PProfile left,
                                  PProfile right) {
  Long e1, e2;
  ...
  FT_TRACE7(("  x=%d y=[% .12f;% .12f]", y, (double)x1 / (double)ras.precision,
             (double)x2 / (double)ras.precision));
  ...
  if (x1 == e1) {
    ...
    if (e1 >= 0 && (ULong)e1 < ras.target.rows) {
      ...
      FT_TRACE7((bits[0] & f1 ? " redundant" : " -> y=%ld edge", e1));
  ...
  if (x2 == e2) {
    ...
    if (e2 >= 0 && (ULong)e2 < ras.target.rows) {
      ...
      FT_TRACE7((bits[0] & f1 ? " redundant" : " -> y=%ld edge", e2));
...
static void Horizontal_Sweep_Drop(RAS_ARGS Short y, FT_F26Dot6 x1,
                                  FT_F26Dot6 x2, PProfile left,
                                  PProfile right) {
  Long e1, e2, pxl;
  PByte bits;
  ...
  if (e1 > e2) {
    ...
    if (e1 == e2 + ras.precision) {
      ...
      if (e1 >= 0 && (ULong)e1 < ras.target.rows && *bits & f1)
        goto Exit;
    } else
      goto Exit;
  }
...
static Bool Draw_Sweep(RAS_ARG) {
  ...
  if (ras.numTurns > 0 && ras.sizeBuff[-ras.numTurns] == min_Y)
    ras.numTurns--;
  ...
  while (ras.numTurns > 0) {
    ...
    while (y < y_change) {
      ...
      while (P_Left && P_Right) {
        ...
        if (x2 - x1 <= ras.precision && e1 != x1 && e2 != x2) {
...
#endif
      ras.Proc_Sweep_Drop(RAS_VARS y, P_Left->X, P_Right->X, P_Left, P_Right);
    }
 ...
 *   Renderer error code.
 */
static int Render_Single_Pass(RAS_ARGS Bool flipped, Int y_min, Int y_max) {
  Int y_mid;
  Int band_top = 0;
...
static int ft_black_new(void *memory, FT_Raster *araster) {
...
static void ft_black_done(FT_Raster raster) {
  /* nothing */
  FT_UNUSED(raster);
}
...
static int ft_black_new(void *memory_,       /* FT_Memory     */
...
static void ft_black_done(FT_Raster raster_) /* black_PRaster */
{
