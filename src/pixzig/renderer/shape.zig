...
pub const ShapeBatchQueue = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, shader: *ManagedShader) !ShapeBatchQueue {
        ...
        batch.attrCoord = @intCast(gl.getAttribLocation(batch.shader.val.program, "coord3d"));
        batch.attrColor = @intCast(gl.getAttribLocation(batch.shader.val.program, "color"));
        batch.uniformMVP = @intCast(gl.getUniformLocation(batch.shader.val.program, "projectionMatrix"));
